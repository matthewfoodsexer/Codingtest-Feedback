...
## 🧠 핵심 접근
이 섹션이 가장 중요합니다. 충분히 깊이 생각해서 이 문제를 푸는 가장 효율적인 알고리즘/자료구조를 찾아내세요. 여러 접근법을 비교한 뒤 최선의 방법을 1~2줄로 설명. (예: "정렬 후 투 포인터 → O(N)")

## ✅ 최적 풀이
(절대 생략하지 마세요. 반드시 완전한 코드를 포함해야 합니다.)
내 코드를 절대 보지 마세요. 존재하지 않는다고 생각하세요. 위 핵심 접근에서 정한 아이디어 + 인터넷/웹에서 찾은 정보를 조합해서 가장 최적의 코드를 직접 만드세요.
...
- 최적 풀이를 만들 때 내 코드를 절대 보지 마세요. 오직 문제 설명 + 핵심 접근 + 인터넷 정보만으로 최적의 코드를 만드세요. 같은 문제를 여러 번 분석해도 항상 같은 최적 풀이를 제시하세요.`;
  },
